...
async function getAdditionalAccountInfo(connectionInfo, logger) {
	...
	try {
		...
		return {
			...
			virtualNetworkRules: _.get(accountData, 'properties.virtualNetworkRules', []).map(({ id, ignoreMissingVNetServiceEndpoint }) => ({
				virtualNetworkId: id,
			...
			locations: _.get(accountData, 'properties.locations', []).map(({ id, locationName, failoverPriority, isZoneRedundant }) => ({
				locationId: id,
				locationName,
				failoverPriority,
				isZoneRedundant
