...
const getIndexKeys = (keys, iterate, isParentActivated) => {
	...
	const commentedKeys = deactivatedKeys ? commentIfDeactivated(deactivatedKeys, { isActivated: false }, true) : '';

	const activatedKeys = dividedKeys.activatedItems.join(', ');

	const delimiter = deactivatedKeys ? ', ' : '';

	return isParentActivated ? activatedKeys + commentedKeys : activatedKeys + delimiter + deactivatedKeys;
};
...
const createMemoryOptimizedIndex = isParentActivated => indexData => {
	...
	if (indexData.hash) {
		...
		index += ` HASH (${activatedKeys}${keysDelimiter}${deactivatedKeys})`;
